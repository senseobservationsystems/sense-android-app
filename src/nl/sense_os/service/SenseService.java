...
public class SenseService extends Service {
	...
	private class ConnectivityListener extends BroadcastReceiver {
		...
		public void onReceive(final Context context, Intent intent) {
			...
			connectionThread.handler.post(new Runnable() {				
				@Override
				public void run() 
				{					
	...
	locationThread, motionThread,phoneStateThread, connectionThread, startThread;
...
//		registerReceiver(screenOffListener, filter);
	...
	{   
		ambienceThread = new ChildThread();
		deviceProxThread = new ChildThread();
		externalSensorsThread = new ChildThread();
		locationThread = new ChildThread();
		motionThread = new ChildThread();
		phoneStateThread = new ChildThread();
		connectionThread = new ChildThread();
		startThread  = new ChildThread();
		
		ambienceThread.start();
		deviceProxThread.start();
		externalSensorsThread.start();
	...
	private void toggleAmbience(boolean active) {
		...
		if (active != isAmbienceActive) {
			...
			if (true == active) {
				...
				ambienceThread.handler.post(new Runnable() {
	...
	private void toggleDeviceProx(boolean active) {
		...
		if (active != isDevProxActive) {
			...
			if (true == active) {
				...
				deviceProxThread.handler.post(new Runnable() {
	...
	private void toggleExternalSensors(boolean active) {
		...
		if (active != isExternalActive) {
			...
			if (true == active) {
				...
				// instantiate the sensor on the main process thread
				externalSensorsThread.handler.post(new Runnable() {
	...
	private void toggleLocation(boolean active) {
		...
		if (active != isLocationActive) {
			...
			if (true == active) {
				...
				locationThread.handler.post(new Runnable() {
					@Override
					public void run() {
						...
						if (true == gps) {
							...
							locMgr.requestLocationUpdates(gpsProvider, time, distance, locListener,locationThread.handler.getLooper());
						}
						...
						if (true == network) {
							...
							locMgr.requestLocationUpdates(nwProvider, time, distance, locListener,
									locationThread.handler.getLooper());
						}
	...
	private void toggleMotion(boolean active) {
		...
		if (active != isMotionActive) {
			...
			if (true == active) {
				...
				// instantiate the sensors on the main process thread
				motionThread.handler.post(new Runnable() {
	...
	private void togglePhoneState(boolean active) {
		...
		if (active != isPhoneStateActive) {
			...
			if (true == active) {
				...
				phoneStateThread.handler.post(new Runnable() {
