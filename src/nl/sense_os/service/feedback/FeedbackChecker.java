...
public class FeedbackChecker extends IntentService {
	...
	private void checkFeedback(String sensorName, String actionAfterCheck) {
		...
		if (null != url) {
			...
			final SharedPreferences authPrefs = getSharedPreferences(
					Constants.AUTH_PREFS, MODE_PRIVATE);
			...
			if (cookie != null) {
				try {
					JSONObject json = SenseApi.getJsonObject(new URI(url),
							cookie);
					// parseFeedback(json);
	...
	private String getFeedbackUrl(String sensor_name) {
		...
		return SenseApi.getSensorUrl(this, sensorName, sensorValue, dataType,
				deviceType);
	}

	/**
	 * Handles the feedback request, starting activities based on the properties
	 * of the feedback
	 * 
	 * @param status
	 ...
	 */
	/*
	 * private void handleFeedback(String status, String action, String uri,
	 * double timestamp) {
	 * 
	 * Log.d(TAG, "Feedback status: " + status); if (status.equals("idle") ||
	 * status.equals("read")) { // do nothing
	 * 
	 * } else if (status.equals("unread")) { Log.d(TAG, "unread feedback: \'" +
	 * action + "\' (" + uri + ")"); try { Intent myAsk = new
	 * Intent("nl.sense_os.myask.FEEDBACK"); myAsk.putExtra("uri", uri);
	 * myAsk.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(myAsk);
	 * unsetFeedback(timestamp); } catch (ActivityNotFoundException e) { String
	 * msg = "Cannot find MyASK activity"; Log.e(TAG, msg); signalError(msg,
	 * timestamp); } catch (Exception e) { String msg =
	 * "Exception starting MyASK activity "; Log.e(TAG, msg, e); signalError(msg
	...
	/**
	 * Called when the service gets in Intent to handle. Looks at the associated
	 * action String to handle the task.
	 */
	@Override
	 ...
	 * 
	 * int total = json.getInt("total"); if (total > 0) { // get feedback value
	 * JSONArray data = json.getJSONArray("data"); JSONObject dataPoint =
	 * data.getJSONObject(0); String value = dataPoint.getString("value");
	 * double date = Double.parseDouble(dataPoint.getString("date"));
	 * 
	 * // strip slashes, re-create JSON sensor value value =
	 * value.replaceAll("\\\\", ""); JSONObject feedback = new
	 * JSONObject(value);
	 * 
	 * // get feedback properties String status = feedback.getString("status");
	 * String action = feedback.optString("action"); String uri =
	 * feedback.optString("uri"); handleFeedback(status, action, uri, date);
	 * 
	 * } else { // Log.d(TAG, "No feedback available (yet)..."); }
	 * 
	 * } catch (JSONException e) { Log.e(TAG,
	 * "JSONException parsing feedback value", e); } catch (Exception e) {
	 ...
	 * 
	 * Map<String, Object> jsonValues = new HashMap<String, Object>();
	 * jsonValues.put("status", "error"); jsonValues.put("action", message);
	 * jsonValues.put("uri", "" + timestamp); JSONObject json = new
	 * JSONObject(jsonValues);
	 * 
	 * // send new feedback sensor value Intent newMsg = new
	 * Intent(MsgHandler.ACTION_NEW_MSG);
	 * newMsg.putExtra(MsgHandler.KEY_SENSOR_NAME, "feedback");
	 * newMsg.putExtra(MsgHandler.KEY_SENSOR_DEVICE, "feedback");
	 * newMsg.putExtra(MsgHandler.KEY_DATA_TYPE,
	 * Constants.SENSOR_DATA_TYPE_JSON); if (timestamp >
	 * System.currentTimeMillis() / 1000) { // make sure we really add this
	 * update as the newest value newMsg.putExtra(MsgHandler.KEY_TIMESTAMP,
	 * (long) ((timestamp + 1) * 1000)); } else {
	 ...
	 * 
	 * Map<String, Object> jsonValues = new HashMap<String, Object>();
	 * jsonValues.put("status", "read"); jsonValues.put("action", "");
	 * jsonValues.put("uri", "" + timestamp); JSONObject json = new
	 * JSONObject(jsonValues);
	 * 
	 * // send new feedback sensor value Intent newMsg = new
	...
	/**
	 * Throw the response of commonSense and it can be catched by the receiver
	 * given with @see actionAfterCheck
	 * 
	 * @param json
	 *            The total response from CommonSense. The JSONObject is set to
	 *            string for sending it with the intent
